...
bool g_bTf2cMatchLogsEnabled = false;
...
static bool WhaleTracker_ExecSchemaQuery(const char[] context, const char[] query)
{
    if (g_hDatabase == null)
    {
        return false;
    ...
    char error[256];
    SQL_GetError(g_hDatabase, error, sizeof(error));
    LogError("[WhaleTracker] Schema query failed (%s): %s | Query: %s", context, error, query);
...
{
    Format(g_sPublicHostIp, sizeof(g_sPublicHostIp), "173.255.237.230");
    RefreshHostCity();
}
...
static void RequestPumpSaveQueue()
{
    if (g_bShuttingDown)
    {
        return;
...
public void OnPluginStart()
{
    ...
    RegConsoleCmd("sm_leaderboard", Command_ShowLeaderboard, "Show WhaleTracker leaderboard page.");
...
public void T_SQLConnect(Database db, const char[] error, any data)
{
    ...
    Format(query, sizeof(query),
        ...
        ... ")");
    if (!WhaleTracker_ExecSchemaQuery("create whaletracker", query))
    {
        return;
    }

    Format(query, sizeof(query), "CREATE INDEX IF NOT EXISTS `idx_last_seen` ON `whaletracker` (`last_seen`)");
    // Best-effort: if unsupported/duplicate, continue with migrations.
    WhaleTracker_ExecSchemaQuery("create whaletracker index", query);
    ...
    Format(query, sizeof(query),
        ...
        ... ")");
    if (!WhaleTracker_ExecSchemaQuery("create whaletracker_online", query))
    {
        return;
    }
        ...
        Format(query, sizeof(query),
            ...
            ... "PRIMARY KEY (`ip`, `port`)"
            ... ")");
    if (!WhaleTracker_ExecSchemaQuery("create whaletracker_servers", query))
    {
        return;
    }
    ...
    if (g_bTf2cMatchLogsEnabled)
    {
        Format(query, sizeof(query),
            ...
            ... ")");
        if (!WhaleTracker_ExecSchemaQuery("create whaletracker_logs", query))
        {
            return;
        }
        ...
        Format(query, sizeof(query),
            ...
            ... ")");
        if (!WhaleTracker_ExecSchemaQuery("create whaletracker_log_players", query))
        {
            return;
        }
    }
    ...
    Format(query, sizeof(query),
        ...
        ... "`updated_at` INTEGER DEFAULT 0"
        ... ")");
    if (!WhaleTracker_ExecSchemaQuery("create whaletracker_points_cache", query))
    {
        return;
    }

    WhaleTracker_ExecSchemaQuery("drop whaletracker_mapstats", "DROP TABLE IF EXISTS `whaletracker_mapstats`");

    // Run migrations/alter steps once after schema bootstrap.
    WhaleTracker_CreateTable(g_hDatabase, null, "", 0);
    WhaleTracker_CreatePointsCacheTable(g_hDatabase, null, "", 0);
}
