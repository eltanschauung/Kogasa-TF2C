...
static bool StartAutoScramble(bool suppressFeedback)
{
    ...
    return StartWhaleScramble(0, !suppressFeedback);
}
...
public int ScrambleVoteHandler(NativeVote vote, MenuAction action, int param1, int param2)
{
    switch (action)
    {
        ...
        case MenuAction_VoteEnd:
        {
            ...
            {
                bool started = StartWhaleScramble(0, true);
                if (started)
                {
...
static bool StartWhaleScramble(int issuer, bool broadcastFailures)
{
    int totalPlayers = 0;
    int redCount = 0;
    int bluCount = 0;
    ...
    {
        NotifyFailure(issuer, broadcastFailures, "Each team needs at least %d eligible non-Engi/Medic players (RED=%d BLU=%d).", swapCount, redEligible, bluEligible);
        return false;
    }
    ...
    CreateTimer(1.0, Timer_DoSwap, pack, TIMER_FLAG_NO_MAPCHANGE);
    return true;
}
...
public Action Timer_DoSwap(Handle timer, DataPack pack)
{
    ...
    if (moved > 0)
    {
        CPrintToChatAll("{tomato}[{purple}Gap{tomato}]{default} Performing a {gold}Whalescramble{default}!");
    }
    else
    {
    ...
    CreateTimer(5.0, Timer_EnableRespawnTimes, _, TIMER_FLAG_NO_MAPCHANGE);
    return Plugin_Stop;
}
